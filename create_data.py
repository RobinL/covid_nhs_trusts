...
df["sum_deaths_rolling_week"] = (
    df.groupby("Name")["num_deaths"].rolling(7).sum().reset_index(0, drop=True)
)
...
df = df.sort_values(["Name", "date"])
df["diff_sum_deaths_rolling_week"] = df.groupby("Name")["sum_deaths_rolling_week"].diff(
    7
)

f1 = df.date == df.date.max()
most_deaths = list(
    df[f1].sort_values("sum_deaths_rolling_week", ascending=False).head(5)["Name"]
)
...
c3 = (c1 + c2).properties(
    height=500, width=800, title="NHS trusts with most deaths in past week"
)
...
greatest_increase = list(
    df[f1].sort_values("diff_sum_deaths_rolling_week", ascending=False).head(5)["Name"]
)
...
c3 = (c1 + c2).properties(
    height=500, width=800, title="NHS trusts greatest increase in deaths in past week"
)
